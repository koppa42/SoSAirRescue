...
SubTaskType = ( Literal['装载'] | Literal['卸货'] | Literal['运送'] | Literal['投放'] | Literal['绞车投放'] | Literal['吊运'] | Literal['卸载']
                | Literal['转移'] | Literal['绞车转移'] | Literal['安置'] | Literal['转运'] | Literal['绞车转运'] | Literal['交接']
                | Literal['取水'] | Literal['灭火'] | Literal['侦查搜寻'] | Literal['加油保障'] )
...
class SubTask:
    def __init__(self, scene: Scene, task_type: SubTaskType, aircraft: Aircraft, position: Position, **kwargs: Unpack[SubTaskParams]) -> None:
        # 所属推演场景
        ...
        if not self.check_valid():
            raise UnsupportedTaskException(f'航空器 {self.aircraft.name} 不能执行 {self.type} 任务')
    ...
    def check_valid(self) -> bool:
        ...
        # 消防
            ...
            if 'load_water' not in self.addition: 
                raise MissingTaskInformationException(f'航空器 {self.aircraft.name} 执行 {self.type} 任务时缺少 load_water 信息')
            if self.addition['load_water'] > self.aircraft.max_external_load:
                raise UnsupportedTaskException(f'航空器 {self.aircraft.name} 无法取水 {self.addition["load_water"]} 吨')
        elif self.type == '灭火':
            if not self.aircraft.ability.can('Fire'): 
                return False
